...
The final predictors is given by: .This process was performed in the auxiliary file `linear_regression.R` by calling `formula <- assemble_linear_formula(train.numeric, predictors, max_pred_number=7, init_formula="score~failures")`.  
...
**Fine-tunned Random Forest, delivered the best performance in terms of both MSE and RMSE. Its boosting mechanism allowed it to effectively minimize the error over multiple rounds, making it the top-performing model in this project.**
...
- Max Verwijmeren: data description, KNN, Random Forest (Iterative parameter adjustments).  
- Nelson Durañona Sosa: data description, Linear regression, code refactoring.
