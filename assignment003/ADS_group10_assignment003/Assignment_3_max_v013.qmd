...
After applying four different models to predict student performance, we now compare the results using **MSE (Mean Squared Error)** as evaluation metric. This metric give us insight into the accuracy of the models, with lower values indicating better performance.
...
Random Forest delivered the best performance in terms of MSE results.

## 6. Finetuning Random Forest

We tried different methods to Finetune to model.
...
```{r}
#Checking Prediction vs True value of Score when with the 'finetuned' model
results_rf <- data.frame(True_Score = RF_labels, Predicted_Score = rf_predictions_cv)
